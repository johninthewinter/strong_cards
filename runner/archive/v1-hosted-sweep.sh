...
OUT="$ROOT/18-opencode-go-sweep-results.jsonl"
...
import json, os, sys
path=sys.argv[1]
coder_rc = os.environ["SWEEP_CODER_RC"]
row={
  ...
  "coder_return_code": int(coder_rc) if coder_rc else None,
...
for model in "${models[@]}"; do
  ...
  if [ "$smoke_rc" -ne 0 ]; then
    ...
    export SWEEP_MODEL="$model"
    export SWEEP_SMOKE_RC="$smoke_rc"
    export SWEEP_SMOKE_VERDICT="$smoke_verdict"
    export SWEEP_SMOKE_TAIL="$(printf '%s' "$smoke_output" | tail -c 4000)"
    python3 - "$OUT" <<'PY'
import json, os, sys
with open(sys.argv[1], "a") as f:
    f.write(json.dumps({
        "model": os.environ["SWEEP_MODEL"],
        "smoke": "FAIL",
        "smoke_return_code": int(os.environ["SWEEP_SMOKE_RC"]),
        "verdict": os.environ["SWEEP_SMOKE_VERDICT"],
        "stdout_stderr_tail": os.environ["SWEEP_SMOKE_TAIL"],
    }) + "\n")
PY
    continue
  fi
